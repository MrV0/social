...
class Post {
    ...
    public function submitPost($body, $user_to, $imageName) {
       ...
       if($check_empty != "" || $imageName != "") {
           ...
           $stopWords = "a about above across after again against all almost alone along already
            ...
            $no_punctuations = preg_replace("/[^a-zA-Z 0-9]+/", "", $body);

            if(strpos($no_punctuations, "height") === false && strpos($no_punctuations, "width") === false
                && strpos($no_punctuations, "http") === false ) {

                $no_punctuations = preg_split("/[\s,]+/", $no_punctuations);

                foreach ($stopWords as $value) {
                    foreach ($no_punctuations as$key => $value2) {
                ...
                foreach ($no_punctuations as $value) {
    ...
    public function calculateTrend($term) {
        ...
        if($term != '') {
            ...
            if(mysqli_num_rows($query) == 0) {
                $insert_quey = mysqli_query($this->con, "INSERT INTO trends(title, hits) VALUES('$term', '1')");
            } else {
                $insert_quey = mysqli_query($this->con, "UPDATE trends SET hits=hits+1 WHERE title='$term'");
    ...
    public function loadPostFriends($data, $limit) {
        ...
        if(mysqli_num_rows($data_query) > 0) {
            ...
            $num_iterations = 0; // Number of results checked (not necasserity posted)
            $count = 1;
